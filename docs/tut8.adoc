...
These methods are used internally by the classes in the materialstore library. Also, these methods are used by the link:https://github.com/kazurayam/inspectus[`inspectus`] library in order to implement what I call **Visual Inspection** --- comparing 2 sets of Material to find differences. These methods encapsulate the complicated processing details; so I would not cover them here in this tutorial document.
